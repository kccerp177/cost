...
function TabSites({ sites, onSelect }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, background: T.bg }}>
      <div style={{ padding: '18px 24px 14px', background: '#fff', borderBottom: `1px solid ${T.lineSoft}`, display: 'flex', alignItems: 'center' }}>
        <div style={{ flex: 1 }}>
      ...
      <div className="no-scrollbar" style={{ flex: 1, overflow: 'auto', padding: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
          {sites.map(site => <TabSiteCard key={site.id} site={site} onClick={() => onSelect(site)}/>)}
        </div>
      </div>
    </div>
  );
